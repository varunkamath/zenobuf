...
impl ZenohTransport {
    ...
    pub async fn create_publisher<M: Message>(&self, topic: &str) -> Result<ZenohPublisher<M>> {
        let prefixed_topic = format!("{prefix}{topic}", prefix = Self::TOPIC_PREFIX, topic = topic);
    ...
    {
        let prefixed_service_name = format!("{prefix}{service_name}", prefix = Self::SERVICE_PREFIX, service_name = service_name);
